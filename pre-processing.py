...
df_ALSFRS_timeseries = df_ALSFRS_timeseries.merge(
    df_DEMO[["subject_id", "Age"]],
    on="subject_id",
    how="left",
)
...
# %% [markdown]
# Calculate average ALSFRS degridation rate per patient

# %%
# Copy dataset
df_ALSFRS_timeseries = df_ALSFRS_timeseries.copy()

# Drop rows with missing values
df_ALSFRS_timeseries = df_ALSFRS_timeseries.dropna(
    subset=["subject_id", "ALSFRS_Total", "timestep"]
)

# Get the baseline (first month) ALSFRS per subject
baselines = df_ALSFRS_timeseries[df_ALSFRS_timeseries["timestep"] == 0][
    ["subject_id", "ALSFRS_Total"]
]
baselines = baselines.rename(columns={"ALSFRS_Total": "ALSFRS_Baseline"})

# Merge baseline back
df_ALSFRS_timeseries = df_ALSFRS_timeseries.merge(
    baselines, on="subject_id", how="left"
)

# Compute decline per month relative to baseline
df_ALSFRS_timeseries["Decline_per_month"] = (
    df_ALSFRS_timeseries["ALSFRS_Baseline"] - df_ALSFRS_timeseries["ALSFRS_Total"]
) / df_ALSFRS_timeseries["timestep"]

# Baseline itself = 0
df_ALSFRS_timeseries.loc[df_ALSFRS_timeseries["timestep"] == 0, "Decline_per_month"] = 0


df_ALSFRS_timeseries.head()
...
# Get each subject's age (assuming age is constant per subject)
patient_ages = df_ALSFRS_timeseries.groupby("subject_id", as_index=False)["Age"].first()

# Assign age bins as integers starting from 1
patient_ages["Age_bin"] = pd.cut(
    patient_ages["Age"], bins=bins, labels=False, right=False
)

# Remove any existing Age_bin column
if "Age_bin" in df_ALSFRS_timeseries.columns:
    df_ALSFRS_timeseries = df_ALSFRS_timeseries.drop(columns=["Age_bin"])

# Merge new Age_bin
df_ALSFRS_timeseries = df_ALSFRS_timeseries.merge(
    patient_ages[["subject_id", "Age_bin"]], on="subject_id", how="left"
)


# %% [markdown]
# Cluster based on ALSFRS degredation rate per age group

# %%

# Copy dataset
df_ts_cluster = (
    df_ALSFRS_timeseries.copy()
)  # df_decline already has 'Decline_per_month'

# Drop rows with missing values for clustering
df_ts_cluster = df_ts_cluster.dropna(subset=["subject_id", "Age", "Decline_per_month"])

# If Age_bin does not exist, create numeric bins
if "Age_bin" not in df_ts_cluster.columns:
    bins = list(range(30, 85, 5))
    patient_ages = df_ts_cluster.groupby("subject_id", as_index=False)["Age"].first()
    patient_ages["Age_bin"] = (
        pd.cut(patient_ages["Age"], bins=bins, labels=False, right=False) + 1
    )
    df_ts_cluster = df_ts_cluster.merge(
        patient_ages[["subject_id", "Age_bin"]], on="subject_id", how="left"
    )

# Compute average decline per subject for clustering
subject_decline = (
    df_ts_cluster.groupby(["subject_id", "Age_bin"])
    .agg(Avg_Decline_per_Month=("Decline_per_month", "mean"))
    .reset_index()
)

# Cluster subjects within each age bin
cluster_list = []
age_bins = subject_decline["Age_bin"].unique()

for bin_label in sorted(age_bins):
    subset = subject_decline[subject_decline["Age_bin"] == bin_label].copy()
    subset = subset.dropna(subset=["Avg_Decline_per_Month"])
    if subset.empty:
        continue

    # Clustering feature
    X = subset[["Avg_Decline_per_Month"]].values
    ...
    cluster_list.append(subset[["subject_id", "Cluster"]])

# Merge cluster labels back into full time series
clusters_all = pd.concat(cluster_list, ignore_index=True)
df_ts_cluster = df_ts_cluster.merge(clusters_all, on="subject_id", how="left")

# df_ts_cluster now has 'Cluster' for each subject
df_ts_cluster.head()


# %% [markdown]
# Save ALSFRS_timeseries with binned Ages and their clusters

# %%
# Save the clustered time series data
output_path_cluster = f"{DATASET_DIR}/processed/ALSFRS_timeseries_clustered.csv"
df_ts_cluster.to_csv(output_path_cluster, index=False)
print(f"Saved clustered time series data to: {output_path_cluster}")
print(f"Shape: {df_ts_cluster.shape}")
